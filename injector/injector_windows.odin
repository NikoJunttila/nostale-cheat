...
inject_dll :: proc(pid: u32, dll_path: string, verbose: bool = false) -> bool {
	...
	dll_path_utf16 := windows.utf8_to_utf16(fullPath)
...
load_local_dll :: proc(dll: string) {
	...
	offset = uintptr(pLocalFunc) - uintptr(local_handle)
	remote_func := uintptr(global_dll_base) + offset
}
// Function prototype for the alert function in the DLL
AlertFunc :: #type proc "stdcall" ()
