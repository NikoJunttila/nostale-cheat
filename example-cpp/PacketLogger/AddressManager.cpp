...
void ReadPattern(const EAddress i_eAddress, const byte *i_abSignature,
                 const char *i_szMask, const DWORD i_dwAdd = 0) {
...
#ifdef ACTIVATE_CONSOLE
  std::cout << std::hex << "[AddressManager] Pattern Result (0x" << i_eAddress
            << "): " << dwAddress << std::endl;
#endif
...
void AddressManager::Initialize() {
  // Recv & Hook
  {
    const byte abSignature[] = {0x55, 0x8B, 0xEC, 0x83, 0xC4, 0xF0, 0x53,
  ...
  {
    // Official
    const byte abSignature[] = {0x53, 0x56, 0x8B, 0xF2, 0x8B, 0xD8, 0xEB,
                                0x04, 0xEB, 0x05, 0x39, 0x19, 0x8B, 0xD6,
                                0x8B, 0xC3, 0xE8, 0xFF, 0xFF, 0xFF, 0xFF,
                                0x84, 0xC0, 0x74, 0x1A};
  ...
  if (!s_mapAddress[EAddress::ASendHook]) {
    const byte abSignature[] = {0x53, 0x56, 0x57, 0x8B, 0xFA, 0x8B, 0xF0,
                                0xB3, 0x01, 0xEB, 0x04, 0xEB, 0x05};
  ...
  {
    const byte abSignature[] = {0xA1, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x00, 0x80,
  ...
  HookManager::Initialize(&HookFunctions::HandleRecvPacket,
                          &HookFunctions::HandleSendPacket);
}
