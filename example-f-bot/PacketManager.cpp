...
void HandleSay(const std::vector<std::string> &packetSplitted) {
  ...
  if (packetSplitted[2] != std::to_string(botManager->getPlayerID()))
    return;
  if (packetSplitted[3] != "10")
    return;
  if (packetSplitted[4] != "fish")
    return;
  if (packetSplitted[5] != "data")
    return;
...
void HandleGuri(const std::vector<std::string> &packetSplitted) {
  ...
  if (packetSplitted.size() < 5)
    return;
  if (packetSplitted[1] != "6")
    return;
  if (packetSplitted[2] != "1")
    return;
  if (packetSplitted[3] != std::to_string(botManager->getPlayerID()))
    return;
